...
function generateTable(firstRowElements, secondRowElements, firstRowEnabled, secondRowEnabled) {
    ...
    for (i in firstRowElements) {
        firstRow.append($("<div class='btn btn-primary btn-number'" + (firstRowEnabled ? "" : " disabled") + ">" + firstRowElements[i] + "</div>"));
    }
    
    var secondRow = $('<div class="btn-group"></div>');
    for (i in secondRowElements) {
        secondRow.append($("<div class='btn btn-danger btn-number'" + (secondRowEnabled ? "" : " disabled") + ">" + secondRowElements[i] + "</div>"));
    }
